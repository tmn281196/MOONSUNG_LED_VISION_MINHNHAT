                        ...
                        "EN", new HelpText
                        {
                            ...
                            Sections = new[]
                            {
                                new Section("1. What the sequence is",
                                    ...
                                    "When the test ends (PASS or NG) the app switches the LED power and all relays OFF. On PASS the jig is raised; on NG it stays down. ROIs are shown only while their VISION CHECK runs; use ALL ROI on the Auto page to see the last results. EMERGENCY STOP switches everything off."),
                                new Section("2. Commands",
                                    ...
                                    "VISION CHECK : the camera checks one LED group. Target = group name (Vision page → LED Group). Timeout = sampling time in ms (default 2000, one sample every 100 ms). By default an ROI is OK when at least one sample shows it lit with the right colour, and the step ends as soon as every ROI is OK, so Timeout is the maximum wait (pass rule on the Setting page; the strict rule needs every sample for the whole time); Persist warm-up samples are not scored."),
                        ...
                        "TH", new HelpText
                        {
                            ...
                            Sections = new[]
                            {
                                new Section("1. ลำดับการทดสอบคืออะไร",
                                    ...
                                    "เมื่อการทดสอบจบ (PASS หรือ NG) แอปจะปิดไฟ LED และรีเลย์ทั้งหมด PASS จะยกจิ๊กขึ้น NG จิ๊กอยู่ล่าง ROI จะแสดงเฉพาะระหว่างที่ VISION CHECK ของมันกำลังรัน ดูผลล่าสุดได้ด้วยปุ่ม ALL ROI ในหน้า Auto EMERGENCY STOP ปิดทุกอย่าง"),
                                new Section("2. คำสั่ง",
                                    ...
                                    "VISION CHECK : กล้องตรวจกลุ่ม LED หนึ่งกลุ่ม Target = ชื่อกลุ่ม (หน้า Vision → LED Group) Timeout = เวลาเก็บตัวอย่างเป็น ms (ค่าเริ่มต้น 2000 เก็บทุก 100 ms) ค่าเริ่มต้น ROI ถือว่า OK เมื่อมีตัวอย่างอย่างน้อยหนึ่งตัวอย่างที่ติดถูกสี และ step จะจบทันทีเมื่อ ROI ทุกอัน OK ดังนั้น Timeout คือเวลารอสูงสุด (เปลี่ยนกฎได้ในหน้า Setting กฎเข้มงวดต้องติดทุกตัวอย่างตลอดเวลา) ตัวอย่างช่วงอุ่นเครื่องของ Persist ไม่ถูกให้คะแนน"),
                        ...
                        "EN", new HelpText
                        {
                            ...
                            Sections = new[]
                            {
                                ...
                                new Section("3. Retest Management",
                                    ...
                                    "VISION CHECK pass rule : ticked = an ROI is OK as soon as one sample during the check shows it lit with the right colour (lenient; use for scanned / multiplexed LED boards where a frame can catch the LED off). Unticked = the ROI must be lit in every sample (strict).",
                        ...
                        "TH", new HelpText
                        {
                            ...
                            Sections = new[]
                            {
                                ...
                                new Section("3. Retest Management",
                                    ...
                                    "VISION CHECK pass rule : ติ๊ก = ROI ถือว่า OK ทันทีที่มีตัวอย่างใดตัวอย่างหนึ่งระหว่างการตรวจเห็นว่าติดถูกสี (ผ่อนปรน ใช้กับบอร์ด LED แบบสแกนที่บางเฟรมอาจจับตอน LED ดับ) ไม่ติ๊ก = ROI ต้องติดในทุกตัวอย่าง (เข้มงวด)",
