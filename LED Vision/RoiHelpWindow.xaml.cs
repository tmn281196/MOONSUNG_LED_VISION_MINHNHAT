                        ...
                        "EN", new HelpText
                        {
                            ...
                            Sections = new[]
                            {
                                ...
                                new Section("2. Commands",
                                    ...
                                    "VISION CHECK : the camera checks one LED group. Target = group name (Vision page → LED Group). Timeout = sampling time in ms (default 2000, one sample every 100 ms). By default (majority rule) an ROI is OK when more samples show it lit with the right colour than not; the step ends early once every ROI has already collected more than half of the possible samples as OK, so Timeout is the maximum wait (rule on the Setting page; the strict rule needs every sample for the whole time). Persist warm-up samples are not scored."),
                                new Section("3. Columns",
                                    ...
                                    "Spec : DELAY time in ms.  Timeout : maximum sampling time of a VISION CHECK (the step ends early once every ROI is OK).",
                        ...
                        "TH", new HelpText
                        {
                            ...
                            Sections = new[]
                            {
                                ...
                                new Section("2. คำสั่ง",
                                    ...
                                    "VISION CHECK : กล้องตรวจกลุ่ม LED หนึ่งกลุ่ม Target = ชื่อกลุ่ม (หน้า Vision → LED Group) Timeout = เวลาเก็บตัวอย่างเป็น ms (ค่าเริ่มต้น 2000 เก็บทุก 100 ms) ค่าเริ่มต้น (กฎเสียงข้างมาก) ROI ถือว่า OK เมื่อจำนวนตัวอย่างที่ติดถูกสีมากกว่าที่ไม่ติด step จะจบก่อนเมื่อ ROI ทุกอันเก็บตัวอย่าง OK ได้เกินครึ่งของจำนวนตัวอย่างสูงสุดแล้ว ดังนั้น Timeout คือเวลารอสูงสุด (เปลี่ยนกฎได้ในหน้า Setting กฎเข้มงวดต้องติดทุกตัวอย่างตลอดเวลา) ตัวอย่างช่วงอุ่นเครื่องของ Persist ไม่ถูกให้คะแนน"),
                                new Section("3. คอลัมน์",
                                    "No : ลำดับการรัน CMD / Sub CMD : คำสั่งและ ON / OFF",
                                    "Target : หมายเลขรีเลย์ (RELAY) หรือชื่อกลุ่ม (VISION CHECK)",
                                    "Spec : เวลา DELAY เป็น ms  Timeout : เวลาเก็บตัวอย่างสูงสุดของ VISION CHECK (step จบก่อนเมื่อ ROI ทุกอัน OK)",
                        ...
                        "EN", new HelpText
                        {
                            ...
                            Sections = new[]
                            {
                                ...
                                new Section("3. Retest Management",
                                    ...
                                    "VISION CHECK pass rule : ticked = majority rule, an ROI is OK when the samples that show it lit with the right colour outnumber the samples that do not (equal = NG); good for scanned / multiplexed LED boards where some frames catch the LED off. Unticked = the ROI must be lit in every sample (strict).",
                        ...
                        "TH", new HelpText
                        {
                            ...
                            Sections = new[]
                            {
                                ...
                                new Section("3. Retest Management",
                                    ...
                                    "VISION CHECK pass rule : ติ๊ก = กฎเสียงข้างมาก ROI ถือว่า OK เมื่อจำนวนตัวอย่างที่เห็นติดถูกสีมากกว่าจำนวนที่ไม่ติด (เท่ากัน = NG) เหมาะกับบอร์ด LED แบบสแกนที่บางเฟรมจับตอน LED ดับ ไม่ติ๊ก = ROI ต้องติดในทุกตัวอย่าง (เข้มงวด)",
