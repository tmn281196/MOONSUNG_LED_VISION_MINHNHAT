                        ...
                        "EN", new HelpText
                        {
                            ...
                            Sections = new[]
                            {
                                ...
                                new Section("2. Add an ROI",
                                    "DOUBLE-click with the LEFT mouse button on an empty spot of the camera image.",
                                ...
                                new Section("4. Select and move many ROIs",
                                    "LEFT-drag on an empty area to draw a rectangle around the ROIs you want; the dashed frame shrinks to the selected ROIs.",
                                    "Or hold Ctrl and click ROIs one by one to add them to (or remove them from) the selection.",
                                    "LEFT-drag inside that frame, or on any selected ROI, to move the whole selection. Arrow keys also move it.",
                                    "Delete, Ctrl+C and Ctrl+D act on the whole selection while the frame is shown.",
                                    "Click on an empty spot, or drag a single ROI, to drop the selection."),
                                ...
                                new Section("7. Zoom the view",
                                    ...
                                    "RIGHT-click on the image to go back to 1:1."),
                                new Section("8. Tips",
                        ...
                        "TH", new HelpText
                        {
                            ...
                            Sections = new[]
                            {
                                ...
                                new Section("2. เพิ่ม ROI",
                                    "ดับเบิลคลิกซ้ายที่จุดว่างบนภาพจากกล้อง",
                                ...
                                new Section("4. เลือกและย้ายหลาย ROI",
                                    "ลากเมาส์ซ้ายบนพื้นที่ว่างเพื่อวาดกรอบล้อม ROI ที่ต้องการ กรอบเส้นประจะหดพอดีกับ ROI ที่เลือก",
                                    "หรือกด Ctrl ค้างแล้วคลิก ROI ทีละอันเพื่อเพิ่มเข้า (หรือเอาออกจาก) กลุ่มที่เลือก",
                                    "ลากเมาส์ซ้ายภายในกรอบนั้น หรือลากที่ ROI ใดก็ได้ที่เลือกอยู่ เพื่อย้ายทั้งกลุ่มพร้อมกัน ปุ่มลูกศรก็ใช้ย้ายได้",
                                    "Delete, Ctrl+C และ Ctrl+D ทำกับทั้งกลุ่มที่เลือกขณะที่กรอบยังแสดงอยู่",
                                    "คลิกที่ว่าง หรือลาก ROI ทีละอัน เพื่อยกเลิกการเลือกกลุ่ม"),
                                ...
                                new Section("7. ซูมภาพ",
                                    ...
                                    "คลิกขวาที่ภาพเพื่อกลับสู่ขนาด 1:1"),
                        ...
                        "EN", new HelpText
                        {
                            ...
                            Sections = new[]
                            {
                                new Section("1. What the sequence is",
                                    ...
                                    "When the test ends (PASS or NG) the app switches the LED power and all relays OFF. On PASS the jig is raised; on NG it stays down. ROIs are shown while their VISION CHECK runs and, when the test ends, all ROIs come back with their result colour (green OK, red NG, grey not checked); the RESULT button opens the combined image. EMERGENCY STOP switches everything off."),
                                new Section("2. Commands",
                                    ...
                                    "VISION CHECK : the camera checks one LED group. Target = group name (Vision page → LED Group). Hold = continuous-OK time in ms an ROI needs (empty = one sample). Timeout = maximum wait in ms (default 2000, one sample every 100 ms). Hold = each ROI must be lit with the right colour continuously for this long (ms) to be OK; empty = one OK sample. Timeout = maximum wait: PASS as soon as every ROI reached Hold, NG for ROIs that never did (rule on the Setting page; the strict rule needs every sample for the whole Timeout). Persist warm-up samples are not scored."),
                                new Section("3. Columns",
                                    ...
                                    "Spec : DELAY time in ms.  Hold : continuous-OK time an ROI needs (VISION CHECK).  Timeout : maximum wait of a VISION CHECK.",
                                ...
                                new Section("4. Editing",
                                    ...
                                    "Default replaces the list with POWER ON → then for every group: RELAY n ON → VISION CHECK → RELAY n OFF (relay 1 for the first group, relay 2 for the second...) → POWER OFF."),
                                new Section("5. Typical sequence (multiplexed LED board: one relay powers one LED group)",
                                    "1) POWER ON",
                                    "2) RELAY ON  Target = 1   → relay 1 powers the common of group 1",
                                    "3) VISION CHECK  Target = 4-LED, Timeout 2000",
                                    "4) RELAY OFF  Target = 1",
                                    "5) RELAY ON  Target = 2  →  VISION CHECK  Target = 7-Segment  →  RELAY OFF  Target = 2",
                                    "6) RELAY ON  Target = 3  →  VISION CHECK  Target = Decimal Point  →  RELAY OFF  Target = 3",
                                    "7) POWER OFF",
                        ...
                        "TH", new HelpText
                        {
                            ...
                            Sections = new[]
                            {
                                new Section("1. ลำดับการทดสอบคืออะไร",
                                    ...
                                    "เมื่อการทดสอบจบ (PASS หรือ NG) แอปจะปิดไฟ LED และรีเลย์ทั้งหมด PASS จะยกจิ๊กขึ้น NG จิ๊กอยู่ล่าง ROI จะแสดงระหว่างที่ VISION CHECK ของมันกำลังรัน และเมื่อการทดสอบจบ ROI ทั้งหมดจะกลับมาแสดงพร้อมสีผลลัพธ์ (เขียว OK แดง NG เทา ยังไม่ตรวจ) ปุ่ม RESULT เปิดภาพรวม EMERGENCY STOP ปิดทุกอย่าง"),
                                new Section("2. คำสั่ง",
                                    ...
                                    "VISION CHECK : กล้องตรวจกลุ่ม LED หนึ่งกลุ่ม Target = ชื่อกลุ่ม (หน้า Vision → LED Group) Hold = เวลาที่ ROI ต้อง OK ต่อเนื่องเป็น ms (ว่าง = หนึ่งตัวอย่าง) Timeout = เวลารอสูงสุดเป็น ms (ค่าเริ่มต้น 2000 เก็บทุก 100 ms) Hold = ROI แต่ละอันต้องติดถูกสีต่อเนื่องนานเท่านี้ (ms) จึงถือว่า OK ว่าง = หนึ่งตัวอย่าง Timeout = เวลารอสูงสุด PASS ทันทีเมื่อ ROI ทุกอันถึง Hold ROI ที่ไม่เคยถึงจะ NG (เปลี่ยนกฎได้ในหน้า Setting กฎเข้มงวดต้องติดทุกตัวอย่างตลอด Timeout) ตัวอย่างช่วงอุ่นเครื่องของ Persist ไม่ถูกให้คะแนน"),
                                new Section("3. คอลัมน์",
                                    "No : ลำดับการรัน CMD / Sub CMD : คำสั่งและ ON / OFF",
                                    "Target : หมายเลขรีเลย์ (RELAY) หรือชื่อกลุ่ม (VISION CHECK)",
                                    "Spec : เวลา DELAY เป็น ms  Hold : เวลาที่ ROI ต้อง OK ต่อเนื่อง (VISION CHECK)  Timeout : เวลารอสูงสุดของ VISION CHECK",
                                ...
                                new Section("4. การแก้ไข",
                                    ...
                                    "Default แทนที่รายการด้วย POWER ON → แล้วสำหรับทุกกลุ่ม: RELAY n ON → VISION CHECK → RELAY n OFF (รีเลย์ 1 สำหรับกลุ่มแรก รีเลย์ 2 สำหรับกลุ่มที่สอง...) → POWER OFF"),
                                new Section("5. ตัวอย่างลำดับทั่วไป (บอร์ด LED แบบสแกน: รีเลย์ 1 ตัวจ่ายไฟ LED 1 กลุ่ม)",
                                    "1) POWER ON",
                                    "2) RELAY ON  Target = 1   → รีเลย์ 1 จ่ายไฟขาร่วมของกลุ่ม 1",
                                    "3) VISION CHECK  Target = 4-LED, Timeout 2000",
                                    "4) RELAY OFF  Target = 1",
                                    "5) RELAY ON  Target = 2  →  VISION CHECK  Target = 7-Segment  →  RELAY OFF  Target = 2",
                                    "6) RELAY ON  Target = 3  →  VISION CHECK  Target = Decimal Point  →  RELAY OFF  Target = 3",
                        ...
                        "EN", new HelpText
                        {
                            ...
                            Sections = new[]
                            {
                                ...
                                new Section("2. Log Directory",
                                    "Folder where the log image of a test is saved (file name = date and time). The image shows the camera view of every VISION CHECK step side by side, each with its step name, value and PASS / FAIL; retest rounds are included.",
                                    "Save log image when the test is NG / PASS : tick which results get an image. Default: NG only.",
                                ...
                                new Section("3. Retest Management",
                                    ...
                                    "VISION CHECK pass rule : ticked = an ROI is OK once it has been lit with the right colour continuously for the Hold time of the step (empty Hold = one sample); the step passes as soon as every ROI is OK and fails if some ROI never gets there within Timeout. Unticked = strict, every sample for the whole Timeout must be OK.",
                    ...
                    },
                    {
                        "TH", new HelpText
                        {
                            ...
                            Sections = new[]
                            {
                                ...
                                new Section("2. Log Directory",
                                    "โฟลเดอร์ที่บันทึกภาพ log ของการทดสอบ (ชื่อไฟล์ = วันที่และเวลา) ภาพจะแสดงมุมมองกล้องของทุก step VISION CHECK เรียงข้างกัน พร้อมชื่อ step ค่า และ PASS / FAIL รวมรอบทดสอบซ้ำด้วย",
                                    "Save log image when the test is NG / PASS : ติ๊กว่าผลแบบไหนให้บันทึกภาพ ค่าเริ่มต้น: NG เท่านั้น",
                                ...
                                new Section("3. Retest Management",
                                    ...
                                    "VISION CHECK pass rule : ติ๊ก = ROI ถือว่า OK เมื่อติดถูกสีต่อเนื่องครบเวลา Hold ของ step (Hold ว่าง = หนึ่งตัวอย่าง) step ผ่านทันทีเมื่อ ROI ทุกอัน OK และล้มเหลวถ้ามี ROI ที่ไม่เคยถึงภายใน Timeout ไม่ติ๊ก = เข้มงวด ทุกตัวอย่างตลอด Timeout ต้อง OK",
