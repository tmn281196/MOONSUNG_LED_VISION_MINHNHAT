                        ...
                        "EN", new HelpText
                        {
                            ...
                            Sections = new[]
                            {
                                ...
                                new Section("2. Commands",
                                    ...
                                    "VISION CHECK : the camera checks one LED group. Target = group name (Vision page → LED Group). Timeout = sampling time in ms (default 2000, one sample every 100 ms). The step always samples for the whole Timeout, then decides: by default an ROI is OK when at least one sample showed it lit with the right colour (rule on the Setting page; the strict rule needs every sample). Persist warm-up samples are not scored."),
                                new Section("3. Columns",
                                    ...
                                    "Spec : DELAY time in ms.  Timeout : sampling time of a VISION CHECK (the step always runs for this long).",
                        ...
                        "TH", new HelpText
                        {
                            ...
                            Sections = new[]
                            {
                                ...
                                new Section("2. คำสั่ง",
                                    ...
                                    "VISION CHECK : กล้องตรวจกลุ่ม LED หนึ่งกลุ่ม Target = ชื่อกลุ่ม (หน้า Vision → LED Group) Timeout = เวลาเก็บตัวอย่างเป็น ms (ค่าเริ่มต้น 2000 เก็บทุก 100 ms) step จะเก็บตัวอย่างจนครบ Timeout เสมอแล้วจึงตัดสิน ค่าเริ่มต้น ROI ถือว่า OK เมื่อมีตัวอย่างอย่างน้อยหนึ่งตัวอย่างที่ติดถูกสี (เปลี่ยนกฎได้ในหน้า Setting กฎเข้มงวดต้องติดทุกตัวอย่าง) ตัวอย่างช่วงอุ่นเครื่องของ Persist ไม่ถูกให้คะแนน"),
                                new Section("3. คอลัมน์",
                                    "No : ลำดับการรัน CMD / Sub CMD : คำสั่งและ ON / OFF",
                                    "Target : หมายเลขรีเลย์ (RELAY) หรือชื่อกลุ่ม (VISION CHECK)",
                                    "Spec : เวลา DELAY เป็น ms  Timeout : เวลาเก็บตัวอย่างของ VISION CHECK (step รันเต็มเวลานี้เสมอ)",
                        ...
                        "EN", new HelpText
                        {
                            ...
                            Sections = new[]
                            {
                                ...
                                new Section("3. Retest Management",
                                    ...
                                    "VISION CHECK pass rule : ticked = an ROI is OK when at least one sample during the whole Timeout shows it lit with the right colour (lenient; for scanned / multiplexed LED boards where some frames catch the LED off). Unticked = the ROI must be lit in every sample (strict).",
                        ...
                        "TH", new HelpText
                        {
                            ...
                            Sections = new[]
                            {
                                ...
                                new Section("3. Retest Management",
                                    ...
                                    "VISION CHECK pass rule : ติ๊ก = ROI ถือว่า OK เมื่อมีตัวอย่างอย่างน้อยหนึ่งตัวอย่างตลอดช่วง Timeout ที่เห็นติดถูกสี (ผ่อนปรน เหมาะกับบอร์ด LED แบบสแกนที่บางเฟรมจับตอน LED ดับ) ไม่ติ๊ก = ROI ต้องติดในทุกตัวอย่าง (เข้มงวด)",
